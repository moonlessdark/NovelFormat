...
class RequestBy:
    ...
    def get(self, url: str, proxies=None, header: dict or str = None) -> ResponseHtml:
        """
        获取页面信息并返回
        ...
        if header is None:
            header = BrowserUserAgent.chrome_macos.c_value
            header = None
        elif type(header) is str:
            for i in BrowserUserAgent:
                if header == i.c_name:
                    header = {"user-agent": i.c_value}
        ...
        time.sleep(random.randint(1, 5))  # 稍微等待以下，减小目标服务器压力

        element = self.re.get(url=url, stream=True, timeout=(20, 300), headers=header)
